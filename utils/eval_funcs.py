...
def top_k_geo(batch, topk, location_decoder, geo_dict):
    """
    count the hit numbers of loc_true in topK of loc_pred, used to calculate Precision, Recall and F1-score,
    calculate the reciprocal rank, used to calcualte MRR,
    calculate the sum of DCG@K of the batch, used to calculate NDCG
    ...
    target_dyna_id = batch['target_dyna_id']
    hit_dict = {}
    predict_dict = {}
    assert topk > 0, "top-k ACC评估方法：k值应不小于1"
    loc_pred = torch.FloatTensor(loc_pred)
    val, index = torch.topk(input=loc_pred, k=topk, dim=1, largest=True, sorted=True)
    index = index.numpy()
    hit = 0
    rank = 0.0
    dcg = 0.0
    ...
    for i, p in enumerate(index):
        ...
        decode_target = location_decoder[target]  #
        decode_p = list(map(location_decoder.get, p))  #
        from geopy.distance import lonlat, geodesic
        candidate_loc_size = len(p)
        for decode_r in decode_p:
            if decode_r is None:  # is pad loc
                pass
            else:
                target_point = geo_dict[decode_target]
                predict_point = geo_dict[decode_r]
                geo_target = lonlat(x=target_point[0], y=target_point[1])
                geo_r = lonlat(x=predict_point[0], y=predict_point[1])
                geo_diff += geodesic(geo_target, geo_r).km

        predict_dict[dyna_id] = decode_p[0]

    return hit, rank, dcg, geo_diff, hit_dict, predict_dict
