...
class LPA(nn.Module):
    """integrate activity semantics in rnn downstream with history attention (DeepMove)"""

    def __init__(self, config, data_feature, embed_layer):
        super(LPA, self).__init__()
        self.loc_size = data_feature['loc_size']
        self.tim_size = data_feature['tim_size']
        self.act_size = data_feature['act_size']
        self.usr_size = data_feature['uid_size']

        loc_pad = data_feature['loc_pad']
        tim_pad = data_feature['tim_pad']

        self.act_type = config['activity_type']
        dropout_p = config['dropout_p']

        self.loc_emb_size = config['loc_emb_size']
        self.tim_emb_size = config['tim_emb_size']
        self.usr_emb_size = config['uid_emb_size']
        ...
        self.emb_act = embed_layer  #
        self.act_emb_size = self.emb_act.embed_size  #
        if self.act_type in ['A3', 'A6', 'L6']:
            input_size = self.loc_emb_size + self.tim_emb_size + self.act_emb_size  #
        else:
        ...
        self.fc_final = nn.Linear(2 * self.hidden_size + self.usr_emb_size, self.loc_size)
    ...
    def forward(self, batch):
        ...
        usr_emb = self.emb_usr(uid)
        if self.act_type in ['L6']:
            act_emb = self.emb_act(loc, downstream=True, pre_len=1, timestamp=tim)  #
        else:
            act_emb = self.emb_act(act, downstream=True, pre_len=1, timestamp=tim)  #

        # change batch * seq * input_size to seq * batch * input_size
        if self.act_type in ['A3', 'A6', 'L6']:
            x = torch.cat((loc_emb, tim_emb, act_emb), 2).permute(1, 0, 2)  #
        else:
        ...
        if self.act_type in ['L6']:
            history_act_emb = self.emb_act(history_loc, downstream=True, pre_len=1, timestamp=history_tim)  #
        else:
            history_act_emb = self.emb_act(history_act, downstream=True, pre_len=1, timestamp=history_tim)  #

        if self.act_type in ['A3', 'A6', 'L6']:
            history_x = torch.cat(
                (history_loc_emb, history_tim_emb, history_act_emb), 2).permute(1, 0, 2)  #
        else:
            history_x = torch.cat(
        ...
        out = torch.cat([out, usr_emb], dim=-1) \
            if self.usr_emb_size > 0 else out  # (N, hidden_size * 2 + user_embed_size)
    ...
    def predict(self, batch):
        ...
        if self.evaluate_method == 'sample':
            # build pos_neg_inedx
            pos_neg_index = torch.cat((batch['target_loc'].unsqueeze(1), batch['neg_loc']), dim=1)
    ...
    def calculate_loss(self, batch):
        ...
        return criterion(scores, batch['target_loc'])